...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const syncFromCookie = async () => {
      // Jangan jalankan sinkronisasi jika URL sedang mengandung token SSO, 
      // agar tidak terjadi race condition di mana sesi dihapus sebelum cookie diset.
      if (window.location.search.includes('token=') || window.location.hash.includes('access_token=')) {
        return;
      }
